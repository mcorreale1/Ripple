...
class EventDescriptionViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        EventManager().eventParticipants(event!) { [weak self] (users, error, event) in
    ...
    func prepareViews() {
        ...
        if let organization = event?["organization"] as? PFObject {
            organization.fetchInBackgroundWithBlock({ (result, error) in
                if(error == nil) {
                    self.nameOrganizationLabel.text = organization["name"] as? String
                } else {
                    print(error)
                }
            })
        } else {
            nameOrganizationLabel.text = ""
        }
    ...
    func prepareNotification() {
        ...
        localNotification.fireDate = event!["startDate"] as? NSDate
        localNotification.alertTitle = event!["name"] as? String
        localNotification.alertBody = event!["description"] as? String
    ...
    @IBAction func goToEventTouch(sender: AnyObject) {
        ...
        refreshAlert.addAction(UIAlertAction(title: okText, style: .Default, handler: { (action: UIAlertAction!) in
            if self.goButton.titleLabel!.text == self.goText {
                self.goButton.setTitle(self.goingText, forState: UIControlState.Normal)
                self.appDelegate.askNotificationsPermissions()
