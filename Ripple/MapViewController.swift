...
class MapViewController: BaseViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    func filteredEvents() {
        ...
        filterEvents = (allEvents as NSArray).filteredArrayUsingPredicate(searchPredicate) as! [PFObject]
    }
    ...
    func showPins() {
        ...
        for event in filterEvents {
            ...
            if let eventLatitude = event["latitude"] as? Float {
                latitude = eventLatitude
            }
            
            if let eventLongitude = event["longitude"] as? Float {
                longitude = eventLongitude
            }
            
            if let title = event["name"] as? String {
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if let eventAnnotation = view.annotation as? EventAnnotation {
            if let event = eventAnnotation.event as? PFObject {
