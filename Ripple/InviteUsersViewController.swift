...
class InviteUsersViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UIScrollViewDelegate {
    ...
    func prepareData() {
        ...
        if organization != nil {
            UserManager().getInvitationOrganization(organization!, completion: { [weak self](users) in
            ...
            if let event = event {
                UserManager().getUserEvent(event, completion: { [weak self](users) in
                    self?.hideActivityIndicator()
                    self?.users = users
                    if let indexUser = self?.users.indexOf(UserManager().currentUser()) {
                        self?.users.removeAtIndex(indexUser)
    ...
    func sendInvitationTouched(objectIndex: NSIndexPath) {
        ...
        showActivityIndicator()
        if organization != nil {
            let acl = self.organization?.ACL
            acl?.setWriteAccess(true, forUser: user)
            self.organization?.ACL = acl
        ...
        if event != nil {
            InvitationManager().sendInvitationInEvent(user, event: event!, completion: {[weak self] (success) in
                self?.hideActivityIndicator()
